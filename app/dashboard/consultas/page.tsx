...
export default async function ClientConsultationsPage() {
  ...
  const { data, error } = await access.sessionClient
    ...
    .eq('empresa_id', access.company.id)
  ...
  const canWrite = access.role !== 'Solo lectura'
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f9] text-[#17324a]">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:py-10">
        ...
        <header className="mt-5"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#a47b24]">Comunicación con SERCOPREV</p><h1 className="mt-2 text-3xl font-black text-[#0f2438] sm:text-4xl">Consultas</h1><p className="mt-2 text-sm text-slate-500">Realice consultas y conserve las respuestas vinculadas a su empresa.</p></header>
        ...
        {canWrite ? <section className="mt-7 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex items-start gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#eaf3f9] text-[#134b78]"><AppIcon name="plus" className="h-5 w-5" /></span><div><h2 className="text-xl font-black text-[#0f2438]">Nueva consulta</h2><p className="mt-1 text-sm text-slate-500">Indique prioridad y contexto completo.</p></div></div><div className="mt-5 max-w-3xl"><NewClientTicketForm /></div></section> : <div className="mt-7 rounded-2xl border border-slate-200 bg-white p-4 text-sm font-bold text-slate-600">Su acceso es de solo lectura. Puede revisar conversaciones, pero no crear ni responder mensajes.</div>}
        <section className="mt-7 grid gap-5">{tickets.length === 0 ? <div className="rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-14 text-center"><AppIcon name="message" className="mx-auto h-9 w-9 text-slate-400" /><p className="mt-4 font-bold text-slate-500">Aún no hay consultas registradas.</p></div> : tickets.map((ticket) => <article key={ticket.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-xl font-black text-[#0f2438]">{ticket.asunto}</h2><StatusBadge status={ticket.prioridad} /></div><p className="mt-1 text-sm text-slate-500">{ticket.categoria} · actualizado {formatDate(ticket.ultimo_mensaje_at, { dateStyle: 'medium', timeStyle: 'short' })}</p></div><StatusBadge status={ticket.estado} /></div><div className="mt-5 grid gap-3 border-l-2 border-slate-200 pl-4">{ticket.mensajes.map((message) => <div key={message.id} className={`rounded-2xl p-4 ${message.autor_tipo === 'SERCOPREV' ? 'bg-[#eaf3f9]' : 'bg-slate-50'}`}><div className="flex items-center justify-between gap-3"><p className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">{message.autor_tipo}</p><p className="text-xs text-slate-400">{formatDate(message.created_at, { dateStyle: 'medium', timeStyle: 'short' })}</p></div><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">{message.mensaje}</p></div>)}</div>{canWrite && ticket.estado !== 'Cerrado' && <ClientTicketReplyForm ticketId={ticket.id} />}</article>)}</section>
      </main>
    </div>
  )
}
