...
import { cambiarEstadoHonorarioNotificado } from '@/app/notified-support-actions'
      ...
      <section className="mt-7 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[1100px] w-full text-left text-sm">
            ...
            <tbody>{fees.length === 0 ? <tr><td colSpan={7} className="px-5 py-14 text-center text-slate-500">No hay honorarios registrados.</td></tr> : fees.map((fee) => <tr key={fee.id} className="border-t border-slate-100 align-top"><td className="px-5 py-4"><Link href={`/admin/clientes/${fee.empresa_id}`} className="font-black text-[#17324a] hover:text-[#134b78]">{companyName(fee.empresa)}</Link>{fee.notas && <p className="mt-1 max-w-xs text-xs leading-5 text-slate-500">{fee.notas}</p>}</td><td className="px-5 py-4 font-bold text-slate-600">{fee.periodo}</td><td className="px-5 py-4 text-slate-600">{fee.concepto}</td><td className="px-5 py-4"><p className="font-bold text-[#17324a]">{formatDate(fee.fecha_vencimiento)}</p><p className="mt-1 text-xs text-slate-500">{fee.estado === 'Pagado' && fee.fecha_pago ? `Pagado ${formatDate(fee.fecha_pago)}` : dueDateLabel(fee.fecha_vencimiento)}</p></td><td className="px-5 py-4 text-right font-black text-[#134b78]">{formatCurrency(fee.monto)}</td><td className="px-5 py-4"><StatusBadge status={fee.estado} /></td><td className="px-5 py-4"><form action={cambiarEstadoHonorarioNotificado} className="flex gap-2"><input type="hidden" name="id" value={fee.id} /><input type="hidden" name="empresa_id" value={fee.empresa_id} /><select name="estado" defaultValue={fee.estado} className="h-9 rounded-lg border border-slate-300 px-2 text-xs font-bold"><option>Pendiente</option><option>Pagado</option><option>Vencido</option><option>Anulado</option></select><button className="h-9 rounded-lg bg-[#0f2438] px-3 text-xs font-black text-white">Guardar</button></form></td></tr>)}</tbody>
          </table>
        </div>
      </section>
    </div>
