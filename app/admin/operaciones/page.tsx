...
export default async function OperationsPage() {
  ...
  const materializationResult = await supabase.rpc('materializar_tareas_recurrentes')
  const [obligationsResult, tasksResult, requestsResult] = await Promise.all([
    ...
    supabase.from('tareas').select('id, empresa_id, titulo, responsable, fecha_vencimiento, estado, prioridad, es_recurrente, periodo_recurrente, empresa:empresas(razon_social)').neq('estado', 'Completada').order('fecha_vencimiento', { ascending: true, nullsFirst: false }).limit(150),
  ...
  const tasks = (tasksResult.data ?? []) as Array<{ id: string; empresa_id: string; titulo: string; responsable: string | null; fecha_vencimiento: string | null; estado: string; prioridad: string; es_recurrente: boolean; periodo_recurrente: string | null; empresa: CompanyRelation }>
  ...
  const error = materializationResult.error || obligationsResult.error || tasksResult.error || requestsResult.error

  return (
    <div className="mx-auto max-w-[1500px]">
      <header><p className="text-xs font-black uppercase tracking-[0.18em] text-[#a47b24]">Control transversal</p><h1 className="mt-2 text-3xl font-black tracking-tight text-[#0f2438] sm:text-4xl">Obligaciones y tareas</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Prioridades de toda la cartera ordenadas por vencimiento y estado. Las series mensuales se preparan automáticamente con anticipación.</p></header>
      {error && <div className="mt-6 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-700">No fue posible cargar o actualizar todas las operaciones.</div>}
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-2">
        ...
        <Panel title="Tareas internas" icon="tasks" count={tasks.length}>
          {tasks.length === 0 ? <Empty text="No hay tareas abiertas." /> : tasks.map((item) => <article key={item.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><Link href={`/admin/clientes/${item.empresa_id}`} className="font-black text-[#17324a] hover:text-[#134b78]">{item.titulo}</Link>{item.es_recurrente && <span className="rounded-full border border-[#134b78]/20 bg-[#eaf3f9] px-2.5 py-1 text-[11px] font-black text-[#134b78]">Mensual</span>}</div><p className="mt-1 text-xs text-slate-500">{nameOf(item.empresa)} · {item.responsable || 'Sin responsable'}{item.periodo_recurrente ? ` · ${formatDate(item.periodo_recurrente, { month: 'long', year: 'numeric' })}` : ''}</p></div><StatusBadge status={item.prioridad} /></div><p className="mt-3 text-xs font-bold text-slate-500">{item.fecha_vencimiento ? `${formatDate(item.fecha_vencimiento)} · ${dueDateLabel(item.fecha_vencimiento)}` : 'Sin vencimiento'}</p><form action={cambiarEstadoTarea} className="mt-4 flex gap-2 border-t border-slate-100 pt-4"><input type="hidden" name="id" value={item.id} /><input type="hidden" name="empresa_id" value={item.empresa_id} /><select name="estado" defaultValue={item.estado} className="h-10 min-w-0 flex-1 rounded-xl border border-slate-300 px-3 text-xs font-bold">{['Pendiente', 'En curso', 'Bloqueada', 'Completada'].map((status) => <option key={status}>{status}</option>)}</select><button className="rounded-xl bg-[#0f2438] px-4 text-xs font-black text-white">Guardar</button></form></article>)}
        </Panel>
      </div>
