...
export default async function AccountingReportsPage({ searchParams }: { searchParams: Promise<{ empresa?: string; periodo?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      <div className="flex flex-wrap gap-3"><Link href={selectedCompany ? `/admin/contabilidad?empresa=${selectedCompany.id}` : '/admin/contabilidad'} className="inline-flex items-center gap-2 text-sm font-black text-[#134b78] hover:underline"><AppIcon name="arrow-right" className="h-4 w-4 rotate-180" />Volver a contabilidad</Link>{selectedCompany && <Link href={`/admin/contabilidad/importaciones?empresa=${selectedCompany.id}`} className="inline-flex items-center gap-2 text-sm font-black text-[#134b78] hover:underline"><AppIcon name="upload" className="h-4 w-4" />RCV y cartolas</Link>}</div>
      <header className="mt-5"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#a47b24]">Libros e informes financieros</p><h1 className="mt-2 text-3xl font-black tracking-tight text-[#0f2438] sm:text-4xl">Reportes contables</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">Balance de comprobación, libro mayor, estado de resultados y balance clasificado construidos únicamente desde asientos contabilizados.</p></header>

      <form method="get" className="mt-7 grid gap-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:grid-cols-[1.3fr_1fr_auto] sm:items-end sm:p-6"><label className="grid gap-2 text-sm font-bold text-slate-700">Empresa<select name="empresa" defaultValue={selectedCompany?.id ?? ''} className="h-11 rounded-xl border border-slate-300 bg-white px-3 text-sm font-bold">{companies.map((company) => <option key={company.id} value={company.id}>{company.nombre_fantasia || company.razon_social} · {company.rut}</option>)}</select></label><label className="grid gap-2 text-sm font-bold text-slate-700">Periodo<select name="periodo" defaultValue={selectedPeriod?.periodo.slice(0, 7) ?? ''} className="h-11 rounded-xl border border-slate-300 bg-white px-3 text-sm font-bold"><option value="">Todos</option>{periods.map((period) => <option key={period.id} value={period.periodo.slice(0, 7)}>{period.periodo.slice(0, 7)} · {period.estado}</option>)}</select></label><button className="h-11 rounded-xl bg-[#0f2438] px-5 text-sm font-black text-white">Generar</button></form>
      ...
      {!selectedCompany ? <Empty text="No hay empresas disponibles." /> : <>
        <section className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric icon="tasks" label="Movimientos" value={movements.length.toString()} /><Metric icon="money" label="Debe" value={formatCurrency(totalDebit)} /><Metric icon="money" label="Haber" value={formatCurrency(totalCredit)} /><Metric icon="check" label="Diferencia diario" value={formatCurrency(totalDebit - totalCredit)} /></section>

        <section className="mt-7 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col gap-3 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6"><div><h2 className="text-xl font-black text-[#0f2438]">Balance de comprobación y saldos</h2><p className="mt-1 text-sm text-slate-500">Periodo {selectedPeriod?.periodo.slice(0, 7) || 'acumulado'} · sólo asientos contabilizados.</p></div><StatusBadge status={Math.abs(totalDebit - totalCredit) < 0.5 ? 'Cuadrado' : 'Descuadrado'} /></div><div className="overflow-x-auto"><table className="min-w-[1050px] w-full text-left text-sm"><thead><tr className="bg-slate-50 text-xs font-black uppercase text-slate-500"><th className="px-4 py-3">Código</th><th className="px-4 py-3">Cuenta</th><th className="px-4 py-3">Tipo</th><th className="px-4 py-3 text-right">Debe</th><th className="px-4 py-3 text-right">Haber</th><th className="px-4 py-3 text-right">Saldo deudor</th><th className="px-4 py-3 text-right">Saldo acreedor</th></tr></thead><tbody>{accounts.length === 0 ? <tr><td colSpan={7} className="p-12 text-center text-slate-500">No hay movimientos contabilizados para el filtro.</td></tr> : accounts.map((account) => <tr key={account.id} className="border-t border-slate-100"><td className="px-4 py-3 font-black text-[#134b78]">{account.code}</td><td className="px-4 py-3 font-bold text-[#17324a]">{account.name}</td><td className="px-4 py-3"><StatusBadge status={account.type} /></td><td className="px-4 py-3 text-right">{formatCurrency(account.debit)}</td><td className="px-4 py-3 text-right">{formatCurrency(account.credit)}</td><td className="px-4 py-3 text-right font-bold">{formatCurrency(account.debtorBalance)}</td><td className="px-4 py-3 text-right font-bold">{formatCurrency(account.creditorBalance)}</td></tr>)}</tbody><tfoot><tr className="border-t-2 border-slate-300 bg-slate-50 font-black"><td colSpan={3} className="px-4 py-4">Totales</td><td className="px-4 py-4 text-right">{formatCurrency(totalDebit)}</td><td className="px-4 py-4 text-right">{formatCurrency(totalCredit)}</td><td className="px-4 py-4 text-right">{formatCurrency(accounts.reduce((sum, item) => sum + item.debtorBalance, 0))}</td><td className="px-4 py-4 text-right">{formatCurrency(accounts.reduce((sum, item) => sum + item.creditorBalance, 0))}</td></tr></tfoot></table></div></section>

        <div className="mt-7 grid gap-6 xl:grid-cols-2"><ReportCard title="Estado de resultados" rows={[['Ingresos', income], ['Gastos', expenses], ['Resultado del periodo', result]]} /><ReportCard title="Balance clasificado resumido" rows={[['Activos', assets], ['Pasivos', liabilities], ['Patrimonio', equity], ['Resultado', result], ['Diferencia de ecuación', balanceDifference]]} warning={Math.abs(balanceDifference) >= 0.5 ? 'La ecuación contable no cierra con la clasificación actual. Revise cuentas de resultado, apertura y cierre.' : null} /></div>

        <section className="mt-7 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 p-5 sm:p-6"><h2 className="text-xl font-black text-[#0f2438]">Libro mayor detallado</h2><p className="mt-1 text-sm text-slate-500">Secuencia de movimientos por cuenta y asiento.</p></div><div className="overflow-x-auto"><table className="min-w-[1100px] w-full text-left text-sm"><thead><tr className="bg-slate-50 text-xs font-black uppercase text-slate-500"><th className="px-4 py-3">Cuenta</th><th className="px-4 py-3">Fecha</th><th className="px-4 py-3">Asiento</th><th className="px-4 py-3">Glosa</th><th className="px-4 py-3 text-right">Debe</th><th className="px-4 py-3 text-right">Haber</th></tr></thead><tbody>{movements.length === 0 ? <tr><td colSpan={6} className="p-12 text-center text-slate-500">Sin movimientos.</td></tr> : [...movements].sort((a, b) => { const accountA = one(a.cuenta)?.codigo ?? ''; const accountB = one(b.cuenta)?.codigo ?? ''; const byAccount = accountA.localeCompare(accountB, 'es', { numeric: true }); if (byAccount !== 0) return byAccount; return (one(a.asiento)?.fecha ?? '').localeCompare(one(b.asiento)?.fecha ?? '') }).map((movement) => { const account = one(movement.cuenta); const entry = one(movement.asiento); return <tr key={movement.id} className="border-t border-slate-100"><td className="px-4 py-3"><p className="font-black text-[#134b78]">{account?.codigo}</p><p className="text-xs text-slate-500">{account?.nombre}</p></td><td className="px-4 py-3">{formatDate(entry?.fecha)}</td><td className="px-4 py-3 font-bold">N° {entry?.numero} · {entry?.tipo}</td><td className="max-w-md px-4 py-3 text-slate-600">{movement.glosa || entry?.glosa}</td><td className="px-4 py-3 text-right">{movement.debe ? formatCurrency(movement.debe) : '—'}</td><td className="px-4 py-3 text-right">{movement.haber ? formatCurrency(movement.haber) : '—'}</td></tr> })}</tbody></table></div></section>

        <section className="mt-7 rounded-3xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-900"><p className="font-black">Alcance del informe</p><p className="mt-1">Estos reportes dependen de la correcta clasificación del plan de cuentas y de asientos contabilizados. No sustituyen la revisión profesional, notas explicativas ni estados financieros formalmente emitidos.</p></section>
      </>}
    </div>
  )
}

function Metric({ icon, label, value }: { icon: 'tasks' | 'money' | 'check'; label: string; value: string }) { return <article className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#eaf3f9] text-[#134b78]"><AppIcon name={icon} className="h-5 w-5" /></span><div><p className="text-xl font-black text-[#0f2438]">{value}</p><p className="text-xs font-bold uppercase text-slate-500">{label}</p></div></article> }
function ReportCard({ title, rows, warning }: { title: string; rows: Array<[string, number]>; warning?: string | null }) { return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><h2 className="text-xl font-black text-[#0f2438]">{title}</h2><div className="mt-5 grid gap-3">{rows.map(([label, value], index) => <div key={label} className={`flex items-center justify-between gap-4 rounded-xl px-4 py-3 ${index === rows.length - 1 ? 'bg-[#0f2438] text-white' : 'bg-slate-50'}`}><span className="font-bold">{label}</span><span className="font-black">{formatCurrency(value)}</span></div>)}</div>{warning && <p className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-xs font-bold leading-5 text-red-800">{warning}</p>}</section> }
