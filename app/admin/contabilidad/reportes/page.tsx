...
export default async function AccountingReportsPage({ searchParams }: { searchParams: Promise<{ empresa?: string; periodo?: string }> }) {
  ...
  const selectedPeriod = params.periodo === 'todos'
    ? null
    : periods.find((item) => item.periodo.slice(0, 7) === params.periodo) ?? periods[0] ?? null
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      <ModulePageHeader
        eyebrow="Contabilidad · Rentabilidad e informes"
        title="Reportes y rentabilidad"
        description="Balance de comprobación, libro mayor, estado de resultados, utilidad o pérdida y balance clasificado construidos exclusivamente desde asientos contabilizados."
        help="Estos reportes son dinámicos y dependen de la clasificación del plan de cuentas. No sustituyen notas explicativas ni estados financieros formalmente emitidos."
        actions={<CompanySelector companies={companies} selectedId={selectedCompany?.id} />}
      />

      <form method="get" className="mt-7 flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-end sm:p-6">
        <input type="hidden" name="empresa" value={selectedCompany?.id ?? ''} />
        <label className="grid min-w-0 flex-1 gap-2 text-sm font-bold text-slate-700">
          Periodo del informe
          <select name="periodo" defaultValue={selectedPeriod?.periodo.slice(0, 7) ?? 'todos'} className="h-11 rounded-xl border border-slate-300 bg-white px-3 text-sm font-bold">
            <option value="todos">Todos los periodos</option>
            {periods.map((period) => <option key={period.id} value={period.periodo.slice(0, 7)}>{period.periodo.slice(0, 7)} · {period.estado}</option>)}
          </select>
        </label>
        <button className="h-11 rounded-xl bg-[#0f2438] px-5 text-sm font-black text-white">Generar informe</button>
      </form>
      ...
      {!selectedCompany ? <Empty text="No hay empresas disponibles." /> : <>
        ...
        <section className="mt-7 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-3 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
            <div>
              <h2 className="inline-flex items-center text-xl font-black text-[#0f2438]">Balance de comprobación y saldos <InfoTip>Suma débitos y créditos por cuenta. Saldo deudor = máximo entre Debe menos Haber y cero. Saldo acreedor = máximo entre Haber menos Debe y cero.</InfoTip></h2>
              <p className="mt-1 text-sm text-slate-500">Periodo {selectedPeriod?.periodo.slice(0, 7) || 'acumulado'} · sólo asientos contabilizados.</p>
            </div>
            <StatusBadge status={Math.abs(totalDebit - totalCredit) < 0.5 ? 'Cuadrado' : 'Descuadrado'} />
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-[1050px] w-full text-left text-sm">
              <thead><tr className="bg-slate-50 text-xs font-black uppercase text-slate-500"><th className="px-4 py-3">Código</th><th className="px-4 py-3">Cuenta</th><th className="px-4 py-3">Tipo</th><th className="px-4 py-3 text-right"><CalculationLabel label="Debe" help="Suma de cargos de la cuenta." /></th><th className="px-4 py-3 text-right"><CalculationLabel label="Haber" help="Suma de abonos de la cuenta." /></th><th className="px-4 py-3 text-right"><CalculationLabel label="Saldo deudor" help="Debe menos Haber cuando el resultado es positivo." /></th><th className="px-4 py-3 text-right"><CalculationLabel label="Saldo acreedor" help="Haber menos Debe cuando el resultado es positivo." /></th></tr></thead>
              <tbody>{accounts.length === 0 ? <tr><td colSpan={7} className="p-12 text-center text-slate-500">No hay movimientos contabilizados para el filtro.</td></tr> : accounts.map((account) => <tr key={account.id} className="border-t border-slate-100"><td className="px-4 py-3 font-black text-[#134b78]">{account.code}</td><td className="px-4 py-3 font-bold text-[#17324a]">{account.name}</td><td className="px-4 py-3"><StatusBadge status={account.type} /></td><td className="px-4 py-3 text-right">{formatCurrency(account.debit)}</td><td className="px-4 py-3 text-right">{formatCurrency(account.credit)}</td><td className="px-4 py-3 text-right font-bold">{formatCurrency(account.debtorBalance)}</td><td className="px-4 py-3 text-right font-bold">{formatCurrency(account.creditorBalance)}</td></tr>)}</tbody>
              <tfoot><tr className="border-t-2 border-slate-300 bg-slate-50 font-black"><td colSpan={3} className="px-4 py-4">Totales</td><td className="px-4 py-4 text-right">{formatCurrency(totalDebit)}</td><td className="px-4 py-4 text-right">{formatCurrency(totalCredit)}</td><td className="px-4 py-4 text-right">{formatCurrency(accounts.reduce((sum, item) => sum + item.debtorBalance, 0))}</td><td className="px-4 py-4 text-right">{formatCurrency(accounts.reduce((sum, item) => sum + item.creditorBalance, 0))}</td></tr></tfoot>
            </table>
          </div>
        </section>
