...
import { OfficialWorkerForm } from '@/app/admin/components/OfficialWorkerForm'
...
type Worker = { id: string; rut: string; nombres: string; apellido_paterno: string; apellido_materno: string | null; email: string | null; telefono: string | null; estado: string; afp: string | null; salud_tipo: string; salud_institucion: string | null; afc_aplica: boolean; asignacion_familiar_tramo: string | null; fecha_ingreso: string }
...
export default async function WorkersPage({ searchParams }: { searchParams: Promise<{ empresa?: string }> }) {
  ...
  const workersResult = selected
    ? await supabase.from('trabajadores').select('id, rut, nombres, apellido_paterno, apellido_materno, email, telefono, estado, afp, salud_tipo, salud_institucion, afc_aplica, asignacion_familiar_tramo, fecha_ingreso').eq('empresa_id', selected.id).order('apellido_paterno')
  ...
  return (
    <div className="mx-auto max-w-[1350px]">
      <ModulePageHeader eyebrow="Remuneraciones · Personas" title="Trabajadores" description="Ficha laboral y previsional. Mantenga aquí sólo los antecedentes permanentes del trabajador; las novedades mensuales se registran en su sección propia." help="La exactitud de AFP, salud, AFC, fecha de ingreso, asignación familiar y centro de costo condiciona todos los cálculos posteriores." actions={<CompanySelector companies={companies} selectedId={selected?.id} />} />
      ...
      {!selected ? <Empty text="No hay empresas disponibles." /> : <>
        ...
        <details className="mt-7 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm open:ring-2 open:ring-[#134b78]/10">
          ...
          <div className="mt-5 border-t border-slate-200 pt-5"><OfficialWorkerForm companyId={selected.id} costCenters={centers} /></div>
        </details>
        ...
        <section className="mt-7 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="overflow-x-auto"><table className="min-w-[1050px] w-full text-left text-sm"><thead><tr className="bg-slate-50 text-xs font-black uppercase text-slate-500"><th className="px-5 py-4">Trabajador</th><th className="px-5 py-4">Ingreso</th><th className="px-5 py-4">Previsión</th><th className="px-5 py-4">Asignación familiar</th><th className="px-5 py-4">Contacto</th><th className="px-5 py-4">Estado</th></tr></thead><tbody>{workers.length === 0 ? <tr><td colSpan={6} className="p-12 text-center text-slate-500">No hay trabajadores registrados.</td></tr> : workers.map((worker) => <tr key={worker.id} className="border-t border-slate-100 align-top"><td className="px-5 py-4"><p className="font-black text-[#17324a]">{worker.nombres} {worker.apellido_paterno} {worker.apellido_materno}</p><p className="mt-1 text-xs text-slate-500">{worker.rut}</p></td><td className="px-5 py-4">{formatDate(worker.fecha_ingreso)}</td><td className="px-5 py-4"><p className="font-bold">{worker.afp || 'AFP pendiente'}</p><p className="mt-1 text-xs text-slate-500">{worker.salud_tipo}{worker.salud_institucion ? ` · ${worker.salud_institucion}` : ''} · AFC {worker.afc_aplica ? 'sí' : 'no'}</p></td><td className="px-5 py-4">{worker.asignacion_familiar_tramo ? `Tramo ${worker.asignacion_familiar_tramo}` : 'Sin tramo'}</td><td className="px-5 py-4"><p>{worker.email || 'Sin correo'}</p><p className="mt-1 text-xs text-slate-500">{worker.telefono || 'Sin teléfono'}</p></td><td className="px-5 py-4"><StatusBadge status={worker.estado} /></td></tr>)}</tbody></table></div>
        </section>
      </>}
    </div>
  )
