...
import { calcularPeriodoConFormulas } from '@/app/admin/payroll-calculation-actions'
...
export default async function PayrollPeriodsPage({ searchParams }: { searchParams: Promise<{ empresa?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[1250px]">
      <ModulePageHeader eyebrow="Remuneraciones · Ciclo mensual" title="Periodos y cálculo" description="Abra un mes sólo cuando sus parámetros legales estén guardados. El cálculo utiliza contrato vigente, novedades mensuales, parámetros versionados y la versión de fórmulas vigente para ese periodo." help="Recalcular reemplaza la liquidación preliminar del periodo. Un periodo cerrado no debe modificarse sin rectificación y trazabilidad. Las fórmulas publicadas se seleccionan por fecha de vigencia y quedan registradas dentro del cálculo." actions={<CompanySelector companies={companies} selectedId={selected?.id} />} />
      ...
      {!selected ? <Empty /> : <>
        <section className="mt-7 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><h2 className="inline-flex items-center text-xl font-black text-[#0f2438]">Abrir nuevo periodo <InfoTip>La apertura vincula el mes con una versión específica de parámetros. Por eso primero debe guardar los parámetros del mismo periodo.</InfoTip></h2><div className="mt-5"><PayrollPeriodForm companyId={selected.id} /></div></section>

        <section className="mt-7 grid gap-4">
          {periods.length === 0 ? <Empty text="No hay periodos abiertos o históricos." /> : periods.map((period) => { const legal = one(period.parametros); return <article key={period.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-xl font-black text-[#0f2438]">{period.periodo.slice(0, 7)}</h2><StatusBadge status={period.estado} />{legal?.indicadores_verificados_at ? <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-black text-emerald-700">Parámetros trazados</span> : <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[11px] font-black text-amber-800">Revisar parámetros</span>}</div><p className="mt-2 text-sm text-slate-500">UF {legal?.uf ?? '—'} · UTM {legal?.utm ?? '—'}{period.calculado_at ? ` · calculado ${formatDate(period.calculado_at, { dateStyle: 'medium', timeStyle: 'short' })}` : ''}{period.cerrado_at ? ` · cerrado ${formatDate(period.cerrado_at)}` : ''}</p><p className="mt-1 text-xs text-slate-400">{legal?.fuente || 'Fuente general no registrada'}</p></div><div className="flex flex-wrap items-center gap-3">{period.estado !== 'Cerrado' && <form action={calcularPeriodoConFormulas}><input type="hidden" name="periodo_id" value={period.id} /><button className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#0f2438] px-5 text-sm font-black text-white"><AppIcon name="settings" className="h-4 w-4" />Calcular nómina</button></form>}<InfoTip title="Motor de cálculo">El cálculo utiliza las fórmulas publicadas cuya fecha de vigencia cubre este periodo. Cada liquidación conserva la expresión y versión aplicada para poder reconstruir el resultado posteriormente.</InfoTip></div></div></article> })}
        </section>
      </>}
    </div>
  )
