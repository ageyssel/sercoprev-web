...
import { cambiarEstadoHonorario, cambiarEstadoTicket } from '@/app/support-actions'
    ...
    <div className="mx-auto max-w-[1300px]">
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-2">
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-5 grid gap-3">{fees.length === 0 ? <Empty text="No hay honorarios registrados." /> : fees.map((fee) => <article key={fee.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-black text-[#17324a]">{fee.periodo} · {fee.concepto}</h3><p className="mt-1 text-xs text-slate-500">{formatDate(fee.fecha_vencimiento)} · {fee.estado === 'Pagado' && fee.fecha_pago ? `pagado ${formatDate(fee.fecha_pago)}` : dueDateLabel(fee.fecha_vencimiento)}</p></div><StatusBadge status={fee.estado} /></div><p className="mt-3 text-xl font-black text-[#134b78]">{formatCurrency(fee.monto)}</p>{fee.notas && <p className="mt-2 text-sm leading-6 text-slate-600">{fee.notas}</p>}<form action={cambiarEstadoHonorario} className="mt-4 flex gap-2 border-t border-slate-100 pt-4"><input type="hidden" name="id" value={fee.id} /><input type="hidden" name="empresa_id" value={id} /><select name="estado" defaultValue={fee.estado} className="h-10 flex-1 rounded-xl border border-slate-300 px-3 text-xs font-bold"><option>Pendiente</option><option>Pagado</option><option>Vencido</option><option>Anulado</option></select><button className="rounded-xl bg-[#0f2438] px-4 text-xs font-black text-white">Guardar</button></form></article>)}</div>
        </section>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-5 grid gap-3">{contacts.length === 0 ? <Empty text="No hay contactos adicionales." /> : contacts.map((contact) => <article key={contact.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-black text-[#17324a]">{contact.nombre}</h3><p className="mt-1 text-xs text-slate-500">{contact.cargo || 'Sin cargo informado'}</p></div>{contact.principal && <StatusBadge status="Activo" className="before:mr-1 before:content-['Principal'] text-transparent before:text-emerald-700" />}</div><div className="mt-3 grid gap-1 text-sm text-slate-600">{contact.email && <a href={`mailto:${contact.email}`} className="font-bold text-[#134b78] hover:underline">{contact.email}</a>}{contact.telefono && <a href={`tel:${contact.telefono.replace(/[^+\d]/g, '')}`} className="font-bold text-[#134b78] hover:underline">{contact.telefono}</a>}<p className="text-xs text-slate-500">Notificaciones: {contact.recibe_notificaciones ? 'sí' : 'no'}</p></div></article>)}</div>
        </section>
      </div>
