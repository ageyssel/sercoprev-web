...
export function OfficialPayrollParametersForm({ companyId, defaults }: { companyId: string; defaults: OfficialPayrollParameterDefaults }) {
  ...
  return (
    <form action={action} className="grid gap-6">
      ...
      <section>
        <h3 className="inline-flex items-center text-base font-black text-[#0f2438]">Indicadores y topes <InfoTip>La UF y UTM se precargan desde SII. Los topes e ingreso mínimo deben corresponder al periodo seleccionado y conservar su fuente oficial.</InfoTip></h3>
      ...
      <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:p-5">
        <h3 className="inline-flex items-center text-base font-black text-[#0f2438]">Tasas totales AFP <InfoTip>Ingrese la cotización obligatoria del trabajador más la comisión vigente de cada AFP como decimal. Ejemplo: 11,44% se registra como 0.1144.</InfoTip></h3>
        <p className="mt-1 text-xs leading-5 text-slate-500">Complete las administradoras vigentes para el periodo. Los trabajadores deben utilizar exactamente uno de estos nombres.</p>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{AFP_FIELDS.map(([label, field]) => <Field key={field} label={label} name={field} inputMode="decimal" placeholder="0.0000" help={`Tasa total de cargo del trabajador afiliado a AFP ${label}.`} />)}</div>
      </section>
      ...
      <Field label="Fuentes y verificación complementaria" name="fuente" defaultValue={defaults.sourceLabel} placeholder="Referencias normativas verificadas" help="Registre aquí fuentes de ingreso mínimo, topes, AFP, SIS y AFC. UF y UTM conservan sus URLs separadas." />
      <p className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs font-semibold leading-5 text-amber-800">UF, UTM y tramos mensuales de Impuesto Único se precargan automáticamente. Ingreso mínimo, topes, AFP, SIS y AFC continúan sujetos a verificación profesional por periodo.</p>
